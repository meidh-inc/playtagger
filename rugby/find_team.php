...
<table border='0' style="width:795px;background-color:#E6E6E6;padding:0px;margin-top:0px;font-family:sans-serif;">
	...
	<td colspan='2' style="height:150px;text-align:left;">
		...
		<?php
		    $get_teams = mysql_query("SELECT name, teamURL FROM teams
					       ORDER BY name ");
		    while($array_teams = mysql_fetch_array($get_teams)){
			$teamName = $array_teams['name'];
			
			$get_coach = mysql_query("SELECT firstName, lastName, teamGender, teamLevel FROM users
						 WHERE userLevel = 'coach' AND teamName = '{$teamName}' ");
			$array_coach = mysql_fetch_array($get_coach);
			$coachName = $array_coach['firstName']." ".$array_coach['lastName'];
			
			$get_teamLocation = mysql_query("SELECT teamID, city, state, teamLevel FROM teams WHERE name = '{$teamName}' ");
			$array_teamLocation = mysql_fetch_array($get_teamLocation);
			$teamID = $array_teamLocation['teamID'];
			$teamLocation = $array_teamLocation['city'].", ".$array_teamLocation['state'];
			
			$get_totalgames = mysql_query("SELECT id FROM gameslist WHERE homeTeam = '{$teamName}' OR awayTeam = '{$teamName}' ");
			$totalgames = mysql_num_rows($get_totalgames);
			
			$get_gameCount = mysql_query("SELECT tagID, gameURL FROM tags
						    WHERE team = '{$teamName}' GROUP BY gameURL ");
			...
			$get_scoreCount = mysql_query("SELECT type FROM tags WHERE type = 'Score' AND team = '{$teamName}' ");
			...
			*/
			echo "<tr>";
			    ...
			    echo "</td>";
			    echo "<td style=\"padding-left:15px;\">";
				echo $array_teamLocation['teamLevel'];
			    echo "</td>";
			    //echo "<td>";
				//echo $coachName;
			    //echo "</td>";
