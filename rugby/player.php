...
if(!isset($_GET['pid'])){
    ...
    $Vpid = $_GET['pid'];
    $get_playerinfo = mysql_query("SELECT email, firstName, lastName, birthday, hsGrad, colGrad, aboutText FROM users
				      WHERE id = '{$_GET['pid']}' ");
    $array_playerinfo = mysql_fetch_array($get_playerinfo);
    
    if($array_playerinfo['firstName'] != ""){
        $playerEmail = $array_playerinfo['email'];
	$firstName = $array_playerinfo['firstName'];
        $lastName = $array_playerinfo['lastName'];
        $birthday = $array_playerinfo['birthday'];
        $hsGrad = $array_playerinfo['hsGrad'];
    ...
    <form action="player.php" method="post">
        <table border='0' style="width:500;height:300px;">
			...
			while($array_messages = mysql_fetch_array($get_messages)){
				...
				$get_sender = mysql_query("SELECT firstName, lastName, teamName, email from users WHERE id = '{$array_messages['fromID']}'");
			    ...
			    echo "<td>".$array_sender['teamName']."</td>";
