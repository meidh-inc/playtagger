...
// NOTE: the 0 is just a place holder for the User ID (CHANGE ONCE SESSIONS ARE UTILIZED)
    ...
    if($sv == "xx"){
        ...
        $get_states = mysql_query("SELECT gameState FROM gameslist GROUP BY gameState ");
        while($array_states = mysql_fetch_array($get_states)){
            echo "<option value=\"".$array_states['gameState']."\">".$array_states['gameState']."</option>";
        }
        echo"</select>";
    }else{
        echo"<select id=\"searchState\" name=\"searchState\">";
        echo"<option value=\"all\">All States</option>";
        $get_statesb = mysql_query("SELECT gameState FROM gameslist WHERE gameCountry = '{$sv}' GROUP BY gameState");
        while($array_statesb = mysql_fetch_array($get_statesb)){
            echo "<option value=\"".$array_statesb['gameState']."\">".$array_statesb['gameState']."</option>";
        }
        echo"</select>";
    }
}
