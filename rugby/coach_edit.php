...
if (isset($_POST['saveCoach'])) { // Form has been submitted.
	...
	$update_coach = "UPDATE users SET
			    ...
			    aboutText = '{$aboutText}'
			    WHERE id = {$userid}";
	$result_coach = mysql_query($update_coach);
...
<table border='0' style="width:795px;background-color:#E6E6E6;padding:0px;margin-top:0px;font-family: sans-serif;">
	...
	<td style="height:50px;text-align:left;">
		...
		<?php
			...
			$get_selectedTeam = mysql_query("SELECT teamName FROM users WHERE id = '{$userid}' ");
			$array_selectedTeam = mysql_fetch_array($get_selectedTeam);
			if($array_selectedTeam['teamName'] == $array_teamName['name']){
			...
			echo "<option value=\"".$array_teamName['name']."\" ".$selectedTeama.">".$array_teamName['name']."</option>";
		    }
	    ...
	    <br>
	    Team Website: <input type="text" name="teamURL" value="<?php echo htmlentities($teamURL); ?>" style="margin-bottom:5px;" /><br>
	    
	    Team Level: 
	    <select name="teamLevel">
		<option value="xx">non-selected</option>
		<option value="High School">High School</option>
		<option value="High School Travel">High School Travel</option>
		<option value="College">College</option>
		<option value="Rep Team">Rep Team</option>
		<option value="Semi-Pro Team">Semi-Pro Team</option>
