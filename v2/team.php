...
if(isset($_GET['tid'])){
    ...
    $check_admin = mysql_query("SELECT adminID FROM teams WHERE teamID = '{$_GET['tid']}' ");
    $array_admin = mysql_fetch_array($check_admin);
    if($array_admin['adminID'] == $userid){
	$teamAdmin = "yes";
    }else{
	$teamAdmin = "no";
    }
    ...
    $get_teaminfo = mysql_query("SELECT name, link, city, state, level, about FROM teams WHERE teamID = '{$_GET['tid']}' ");
    ...
    $teamLevel = $array_teaminfo['level'];
    $teamURL = $array_teaminfo['link'];
    $aboutText = $array_teaminfo['about'];
...
<table border='0' style="width:795px;background-color:#E6E6E6;padding:0px;margin-top:0px;font-family:sans-serif;">
		...
		<tr>
			...
			<?php
				...
				//Count Unread Messages
				    $get_messageCount = mysql_query("SELECT messageID FROM messages WHERE toTeamID = '{$tid}' AND isRead != 'read' ");
	...
	<td colspan='3' style="height:50px;text-align:left;vertical-align:text-top;padding:20px;">
	    Coaches<br><br>
	    <?php
		$get_coachList = mysql_query("SELECT userID, teamGender FROM userinfo WHERE teamID = '{$tid}' ");
		while($array_coachList = mysql_fetch_array($get_coachList)){
		    
		    $get_moreCoachInfo = mysql_query("SELECT userLevel, firstName, lastName FROM userlog WHERE userID = '{$array_coachList['userID']}' ");
		    $array_moreCoachInfo = mysql_fetch_array($get_moreCoachInfo);
		    if($array_moreCoachInfo['userLevel'] == 'coach'){
			...
			echo "<a href=\"coach.php?pid=".$array_coachList['userID']."\">".$array_moreCoachInfo['firstName']." ".$array_moreCoachInfo['lastName']."</a>, ".$teamGen."<br>";
