...
if(isset($_GET['s'])){ //get sport, set variable
    $sportID = $_GET['s'];
}else{
    $sportID = 0;
}
$get_sportname = mysql_query("SELECT sportName FROM sports WHERE sportID = '{$sportID}' ");
    $array_sportname = mysql_fetch_array($get_sportname);
    if($array_sportname['sportName'] == ""){
	$sportName = "ERROR: No Sport Detected";
    }else{
	$sportName = $array_sportname['sportName'];
    }
...
if(isset($_GET['kid'])){ //user might be logging in for the first time, this is signup verification check
    $userid = $_GET['uid'];
    $signup_key = $_GET['kid'];
    $check_verification = mysql_query("SELECT verified FROM userlog WHERE userID = '{$userid}' ");
	...
	if($array_verification['verified'] == $signup_key){  // account verified!
	    $update_verified = "UPDATE userlog SET verified = 'yes' WHERE userID = '{$userid}' ";
            $result_verified = mysql_query($update_verified);
	    
	    //get redirect from sports table
	    if($sportID == 1){  //rugby
		redirect_to('http://www.worldrugbyshop.com/playtagger.html');
	    }elseif($sportID == 2){ //volleyball
		redirect_to('http://volleyball.playtagger.com/');
	    }elseif($sportID == 3){  //football
		redirect_to('http://www.sportsspotlight.com/playtagger.cfm');
	    }elseif($sportID == 4){  //basketball
		redirect_to('http://basketball.playtagger.com/');
	    }
...
if (isset($_POST['login'])) { // The user clicked 'login'
	...
	$sportIDL = trim(mysql_prep($_POST['sportID']));
    ...
    // no errors detected so far, continue
		...
		$query_login = "SELECT userID, email, verified, userLevel, firstName, paidDate, expDate FROM userlog WHERE email = '{$email_log}'
		...
		if (mysql_num_rows($result_login) == 1) {  // Success
			...
			if($found_user['userLevel'] == 'admin'){
                            ...
                            redirect_to("video.php".$found_user['userID']);
                        }
	...
	$query_verify_email = mysql_query("SELECT * FROM userlog WHERE email = '$email_sign' AND verified != 'invited' ");
    ...
    // email and password are matching and verified
		...
		// Enter user info into DB
                $check_invite = mysql_query("SELECT verified FROM userlog WHERE email = '{$email_sign}' ");
                ...
                if($array_invite['verified'] == 'invited'){   //new user was invited, update their invite temp profile
                    ...
                    $query_sign = "INSERT INTO userlog ( email, verified, userLevel, hashed_password, signup_date
...
$get_uid = mysql_query("SELECT userID FROM userlog WHERE email = '{$email_sign}' AND hashed_password = '{$hashed_password}' AND signup_date = '{$timestamp_sign}' ");
$array_uid = mysql_fetch_array($get_uid);
$uid = $array_uid['userID'];

//insert row into userinfo
$insert_newSport = mysql_query("INSERT INTO userinfo (userID, sportID) VALUES ('{$uid}', '{$sportID}') ");
...
$body = "
	...
	Please follow the link to <a href=\"http://playtagger.com/v2/index.php?uid=$uid&kid=$key&s=$sportID\">VERIFY</a> your email and log in for the first time.
	<br>
...
// Send them to video landing page if they have not returned from video.php
if((!isset($_SESSION['userid'])) && (!isset($_GET['l']))){
    $_SESSION['userid'] = "xxv";
    $_SESSION['paid'] = "notpaid";
    $_SESSION['sport'] = $sportID;
    $_SESSION['sportName'] = $sportName;
    redirect_to('video.php');
}elseif(($_SESSION['userid'] == "xxv") && (!isset($_GET['l']))){
    $_SESSION['userid'] = "xxv";
    $_SESSION['paid'] = "notpaid";
    $_SESSION['sport'] = $sportID;
    $_SESSION['sportName'] = $sportName;
...
// Send them to video.php if they are logged in and remembered
if((isset($_SESSION['userid'])) && (isset($_SESSION['paid'])) && ($_SESSION['userid'] != "xxv")){
    $_SESSION['sport'] = $sportID;
    $_SESSION['sportName'] = $sportName;
    redirect_to('video.php');
}
...
<div style="width:725px;font-family: sans-serif;">
	...
	<div class="left" style="float: left;">
	    <img src="images/playtagger_logo_lg.png" alt="Playtagger Logo" id="vidpop" style="background: none;border:none;border-top-right-radius:0px;border-top-left-radius:0px;border-bottom-right-radius:0px;border-bottom-left-radius:0px;">
	</div>
	<div class="right" style="float:left;width:300px;text-align:left;padding:30px;">
		...
		<br><br>
		<span style="color:#9BC21B;font-weight:bold;">Click the logo to see the video!</span>
	</div>
    </div>
    <div style="height:25px;"></div>
    <div class="ui-corner-all" style="background-color:#E6E6E6;padding: 20px;">
