...
if(!isset($_GET['pid'])){
    ...
    $Vpid = $_GET['pid'];
    $get_playerinfo1 = mysql_query("SELECT email, firstName, lastName FROM userlog
				      WHERE userID = '{$_GET['pid']}' ");
    $array_playerinfo1 = mysql_fetch_array($get_playerinfo1);
    $get_playerinfo2 = mysql_query("SELECT birthday, hsGrad, colGrad, aboutText FROM userinfo
				      WHERE userID = '{$_GET['pid']}' AND sportID = '{$sportID}' ");
    $array_playerinfo2 = mysql_fetch_array($get_playerinfo2);
    
    if($array_playerinfo1['firstName'] != ""){
        $playerEmail = $array_playerinfo1['email'];
	$firstName = $array_playerinfo1['firstName'];
        $lastName = $array_playerinfo1['lastName'];
        $birthday = $array_playerinfo2['birthday'];
        $hsGrad = $array_playerinfo2['hsGrad'];
    ...
    //look up viewer info
    $get_Vinfo = mysql_query("SELECT email, firstName, lastName FROM userlog
				      WHERE userID = '{$userid}' ");
...
if(isset($_POST['send'])){
	...
	redirect_to("player.php?pid=$Vpid");
}
...
if(isset($_POST['messageclose'])){
	...
	$readNAM = $array_messagesList['messageID'];
	
	if(isset($_POST['read'.$readNAM])){  //mark it read
	    $readVAL = trim(mysql_prep($_POST['read'.$readNAM]));
	    $update_messages = mysql_query("UPDATE messages set isRead = 'read' WHERE messageID = {$readVAL}");
	}elseif((!isset($_POST['read'.$readNAM]))&&($array_messagesList['isRead'] == 'read')){  //mark it unread
	    $update_messages2 = mysql_query("UPDATE messages set isRead = '' WHERE messageID = {$readNAM}");
	}
...
// button to navigate to videos page
if(isset($_POST['toMyGames'])){
    if(isset($_GET['pid'])){  //Viewing another Player's page
	$get_pid = $_GET['pid'];
    	redirect_to("video.php?pid=$get_pid");
    }else{
	redirect_to("video.php?pid=$userid");
    }
}
    ...
    <div id="pay"><center>
        <table border='0' style="width:450;height:400px;">
			...
			if($sportID == 1){
			    ...
			    echo "<button id=\"payclose\" type=\"button\" value=\"Thank You\" >Thank You</button>";
			}
		    ?>
...
<div id="dialog-contact" title="Send <?php echo $firstName; ?> the following message:">
    <div id="contact"><center>
        <table border='0' style="width:400;height:200px;">
		    ...
		    Check out my videos and feel free to contact me at <a href="mailto:<?php echo $Vemail; ?>?Subject=Contacted%20on%20PlayTagger" style="text-decoration:underline;background:none;border:none;"><?php echo $Vemail; ?></a>.<br>
		    <br>
...
<div id="dialog-messages" title="Message Viewer">
    ...
    <form action="player.php" method="post">
        <table border='0' style="width:500;height:300px;">
            <tr>
                <td>
			...
			while($array_messages = mysql_fetch_array($get_messages)){
				...
				$get_sender = mysql_query("SELECT email, firstName, lastName from userlog WHERE userID = '{$array_messages['fromID']}'");
				$array_sender = mysql_fetch_array($get_sender);
			    
			    //look up teamID
				$get_teamID = mysql_query("SELECT teamID from userinfo WHERE userID = '{$array_messages['fromID']}'");
				$array_teamID = mysql_fetch_array($get_sender);
			    
			    //look up teamName
				$get_senderTeamName = mysql_query("SELECT name from teams WHERE teamID = '{$array_teamID['teamID']}'");
				$array_senderTeamName = mysql_fetch_array($get_sender);
			    ...
			    //Get message id
				$messageID = $array_messages['messageID'];
			    ...
			    echo "<td>".$array_senderTeamName['name']."</td>";
			    echo "<td>".$array_sender['email']."</td>";
			    echo "<td>".date('M d, Y', strtotime($array_messages['sentDate']))."</td>";
			    echo "</tr>";
...
<center>
<div style="width:725px;font-family: sans-serif;">
    <div style="height:20px;"></div>
    <div style="height:60px;">
	<div class="ui-corner-all" style="background-color:#000000;padding:5px;text-align: right;">
		...
		if(isset($_GET['pid'])){  //Viewing another Player's page
		    echo "<td>";
		    echo "<div class=\"video\"><button type=\"button\" name=\"toMyGames\" value=\"MyGames\" style=\"cursor:pointer;height:46px;width:136px;border-color:#555555;font-size:.9em;font-family: Verdana,Arial,sans-serif;\">My Games</button></div>";
		    echo "</td>";
		    //Show Contact Me Button
		    echo "<td>";
		    echo "<div class=\"contact\"><button id=\"contactpop\" name=\"contactme\" value=\"Contact Me\" style=\"cursor:pointer;height:46px;width:136px;border-color:#555555;font-size:.9em;font-family: Verdana,Arial,sans-serif;\">Contact Me</button></div>";
		    echo "</td>";
		    ...
		    $get_messageCount = mysql_query("SELECT messageID FROM messages WHERE toID = '{$userid}' AND isRead != 'read' ");
		    $messageCount = mysql_num_rows($get_messageCount);
		    if($messageCount == 1){$notice = "Message";}else{$notice = "Messages";}
		    
		    //Show Messages Button
		    echo "<td>";
		    echo "<div class=\"messages\"><button id=\"messagespop\" name=\"messages\" value=\"View Messages\" style=\"cursor:pointer;height:46px;width:136px;border-color:#555555;font-size:.8em;font-family: Verdana,Arial,sans-serif;\">$messageCount new $notice</button></div>";
		    echo "</td>";
		}
		?>
		<td>
		    <form action="player.php" method="post">
		    <div class="video"><button type="submit" name="toVideos" value="PlayTagger" style="cursor:pointer;height:46px;width:136px;border-color:#555555;font-size:.9em;font-family: Verdana,Arial,sans-serif;">All Videos</button></div>
		    </form>
		</td>
		<td>
		<?php
			...
			echo "
			<form action=\"player.php\" method=\"post\">
			<div class=\"edit\"><button type=\"submit\" name=\"editMe\" value=\"EditProfile\" style=\"cursor:pointer;height:46px;width:136px;border-color:#555555;font-size:.9em;font-family: Verdana,Arial,sans-serif;\">Edit Profile</button></div>
			</form>
			";
		    }
		?>
		...
		<td>
		    <form action="player.php" method="post">
		    <div class="logout"><button type="submit" name="logout" value="logout" style="cursor:pointer;height:46px;width:136px;border-color:#555555;font-size:.9em;font-family: Verdana,Arial,sans-serif;">Logout</button></div>
		    </form>
		</td>
		</tr>
	    </table>
	...
	<div class="right" style="float:left;width:300px;text-align:left;padding:30px;">
	    <span style="font-size:26px;">
	    <?php echo $firstName; ?> <?php echo $lastName; ?></span><br>
	    Age: <?php echo $age; ?><br>
	    High School Grad Year: <?php echo $hsGrad; ?><br>
	    College Grad Year: <?php echo $colGrad; ?><br>
	</div>
    </div>
    <div style="text-align: left;">
	About Me: <?php echo nl2br($aboutText); ?>
    </div>
    <div style="height: 60px;"></div>
</div>
...
<table border='0' style="width:795px;padding:0px;margin-top:0px;font-family:sans-serif;">
    <tr>
