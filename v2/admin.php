...
if (isset($_POST['saveGame'])) { // Form has been submitted.
	...
	$insertGame = "INSERT INTO games (
					    hometeamID, hometeamScore, awayteamID, awayteamScore, gender, city,
					    state, country, date, koHR, koMIN, koAP, link, ytchar
					    ) VALUES (
					    '{$homeID}', '{$homeScore}', '{$awayID}', '{$awayScore}', '{$gameGender}',
...
if(isset($_POST['saveCoaches'])){
    $get_teamList = mysql_query("SELECT teamID, name, adminID FROM teams");
...
<table border='0' style="width:795px;background-color:#E6E6E6;padding:0px;margin-top:0px;">
	...
	<td colspan='2' style="text-align:left;">
		...
		<div id="tabs-2">
			...
			<?php
			    $get_games = mysql_query("SELECT gameID, hometeamID, awayteamID, gender, date, koHR, koMIN, koAP FROM games");
			    $count_games = mysql_num_rows($get_games);
			    if($count_games >= 1){
			    while($array_games = mysql_fetch_array($get_games)){
				//get the team names
				    $get_awayteamName = mysql_query("SELECT teamID, name FROM teams WHERE teamID = '{$array_games['awayteamID']}' ");
				    $array_awayteamName = mysql_fetch_array($get_awayteamName);
				    
				    $get_hometeamName = mysql_query("SELECT teamID, name FROM teams WHERE teamID = '{$array_games['hometeamID']}' ");
				    $array_hometeamName = mysql_fetch_array($get_hometeamName);
				    
				    
				echo "<tr>";
				    echo "<td>".$array_hometeamName['name']." VS ".$array_awayteamName['name']." (".$array_games['gender'].")</td>";
				    echo "<td>Game Day: ".$array_games['date'].", KO: ".$array_games['koHR'].":".$array_games['koMIN']." ".$array_games['koAP']."</td>";
				    echo "<td> (view) (edit) </td>";
				echo "</tr>";
			    }
			    }else{
		...
		<div id="tabs-5">
			...
			<?php
			    $get_teamsList = mysql_query("SELECT teamID, name, city, state, adminID FROM teams");
				...
				echo "<td><select id=\"".$array_teamsList['teamID']."\" name=\"".$array_teamsList['teamID']."\"><option value=\"0\">none</option>";
				    ...
				    $get_coachList = mysql_query("SELECT infoID, userID FROM userinfo WHERE teamID = '{$array_teamsList['teamID']}' ");
				    while($array_coachList = mysql_fetch_array($get_coachList)){
					//get coach names
					$get_coachID = mysql_query("SELECT firstName, lastName from userlog WHERE userID = '{$array_coachList['userID']}' ");
					$array_coachID = mysql_fetch_array($get_coachID);
					
					//determine if this coach is currently the admin
					if($array_teamsList['adminID'] == $array_coachList['userID']){
					    $s1 = "selected=\"selected\"";
					}else{
					    $s1 = "";
					}
					//echo dropdown options
					echo "<option value=\"".$array_coachList['userID']."\" ".$s1.">".$array_coachID['firstName']." ".$array_coachID['lastName']."</option>";
				    }
				echo "</select></td>";
				echo "</tr>";
			    }
